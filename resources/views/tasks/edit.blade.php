                    ...
                    <form action="{{ route('tasks.update', $task) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nombre') is-invalid @enderror" 
                                id="nombre" name="nombre" value="{{ old('nombre', $task->nombre) }}" required>
                            @error('nombre')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('descripcion') is-invalid @enderror" 
                                id="descripcion" name="descripcion" rows="3">{{ old('descripcion', $task->descripcion) }}</textarea>
                            @error('descripcion')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select class="form-select @error('proyecto_id') is-invalid @enderror" 
                                ...
                                @foreach($projects as $project)
                                    <option value="{{ $project->id }}" 
                                        {{ old('proyecto_id', $task->proyecto_id) == $project->id ? 'selected' : '' }}>
                                        {{ $project->nombre }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('desarrollado_por') is-invalid @enderror" 
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" 
                                        {{ old('desarrollado_por', $task->desarrollado_por) == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('tiempo_estimado') is-invalid @enderror" 
                                        id="tiempo_estimado" name="tiempo_estimado" 
                                        value="{{ old('tiempo_estimado', $task->tiempo_estimado) }}" min="0">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('tiempo_real') is-invalid @enderror" 
                                        id="tiempo_real" name="tiempo_real" 
                                        value="{{ old('tiempo_real', $task->tiempo_real) }}" min="0">
                                    @error('tiempo_real')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
